...
var generator = function* generator(next)
{
    ...
    if (options.on)
    {
        var reqBody = yield parse.json(this,
        {
            limit: '1kb'
        });
        genrateApiCalls(this.req, this, this.body, reqBody);
    }
};

function genrateApiCalls(req, o, chunk, reqBody)
{
    var call = {
        "Id": 0,
        "CurrentPath": req.url,
        ...
        "ResponseHeader":
        {
            'Content-Type': o.get('Content-Type')
        },
        "RequestUrlParams": req._parsedUrl.query,
        "RequestBody": JSON.stringify(reqBody),
        "ResponseBody": JSON.stringify(chunk),
        "ResponseCode": o.response.status
    };
